...
function createModalMarkup(accident) {
  return `
    <img class="small-img" src="${accident.images?.[7]?.url || accident.images?.[0]?.url || ''}" alt="">
    <div class="content-container">
    <img class="big-img" src="${accident.images?.[4]?.url || accident.images?.[0]?.url || ''}" alt="">
    
    <div class="info-container">  
      <h3 class="title-info">INFO</h3>
      <p>${accident.promoter?.description || "No description available"}</p>

      <h3 class="title-when">WHEN</h3>         
      <p>${accident.dates?.start?.localDate || "No date"}</p>         
      <p>${accident.dates?.start?.localTime || ""} (${accident.dates?.timezone || ""})</p>         

      <h3 class="title-where">WHERE</h3>         
      <p>${accident._embedded?.venues?.[0]?.city?.name || ""}, 
         ${accident._embedded?.venues?.[0]?.country?.name || ""}</p>         
      ...
      <p>
        ${accident.priceRanges?.[0]?.type || ""} 
        ${accident.priceRanges?.[0]?.min || ""} -
        ${accident.priceRanges?.[0]?.max || ""} 
        ${accident.priceRanges?.[0]?.currency || ""}
      </p>         
